...
def PtIr(fig=None, PlotSelect=None, Vz=None):
    ...
    if Vz is None:
        Vz = -4e-4  # offset voltage on the scope, often different between measurements    Iz = Vz/R  # offset current calculated form the resistance
    ...
    def fig1():
        ...
        for n in range(1, 25):
            ...
            plt.plot([min(T[PS]), max(T[PS])], [In, In], 'r', lw=0.5)
    ...
    def fig2():
        ...
        plt.plot(T[PS], G[PS]/G0, label='PtIr')

        plt.xlabel('Time [s]')
        plt.ylabel('Conductance [2e^2/h]')

        for n in range(1, 25):
            plt.plot([min(T[PS]), max(T[PS])], [n, n], 'r', lw=0.5)
