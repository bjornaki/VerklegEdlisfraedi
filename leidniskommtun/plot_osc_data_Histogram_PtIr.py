...
def PtIr(fig=None, PlotSelect=None, Vz=None, axes1=None, axes2=None, axes3=None,axesNo=0, fig4 = None):
    ...
    def fig4(axes1=axes1,axes2=axes2,axes3=axes3,axSelect = axesNo,fig4 = fig4):
        # Figure 4 is a histogram from the Conductance data
        if axes1==None:
            fig4, (ax1, ax2, ax3) = plt.subplots(1,3,sharey=True, sharex=True,figsize=(10,5))
        else:
            ax1 = axes1
            ax2 = axes2
            ax3 = axes3
        ...
        ax3.hist(TOT_G/G0, bins=bins, range=binrange, align='right',edgecolor='b',alpha=0.7)
        #ax2.set_xlabel(r'Leiðni [2e$^2$/h]')
        #ax2.set_ylabel('Tíðni [fjöldi]')

        Height = np.array([365,210,230,250,245,235,270])
        #Adding expected lines for the expected data
        for n in range(1, 8):
            ax3.plot([n, n], [0, 1000], 'r--')

        ax3.axis([-0.5, 8, 0, 1000])

        ax3.text(1.1,800,"C",size=24)
        ax3.set_xticks([0,1,2,3,4,5,6,7,8])
        ax3.tick_params(right=True,top=True,direction='in',labelsize=20,size=14)
    ...
    #fig4()
    plt.savefig("PtIr_Cond_Histogram.png",format="png",dpi=300,bbox_inches="tight")
    #plt.show()
